...
public class UsuarioWindow extends MainWindow<Usuario>{
	...
	public UsuarioWindow() {
		super(new Usuario("Marcelo", "Lopez", "36747012", new Estandar(), new Aterrizar(null)));
	}
	...
	public void createContents(Panel panelPrincipal) {
		...
		Panel panelSaludo = new Panel(panelPrincipal).setLayout(new HorizontalLayout());
		new Label(panelSaludo).setText("Hola "); new Label(panelSaludo).bindValueToProperty("nombre");
		Panel panelPregunta = new Panel(panelPrincipal).setLayout(new HorizontalLayout());
		new Label(panelPregunta).setText("¿Qué desea hacer?");
		Panel panelBotones = new Panel(panelPrincipal).setLayout(new ColumnLayout(3));
